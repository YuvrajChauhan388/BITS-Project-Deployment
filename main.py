...
with container:
    st.write(f"<h1 style='text-align: center;'>Multi-File Signal Analysis</h1>", unsafe_allow_html=True)

    uploaded_files = st.file_uploader("Choose LVM files", type=['lvm', 'txt'], accept_multiple_files=True)
    ...
    if st.session_state.uploaded_files:
        ...
        df = pd.read_csv(pd.compat.StringIO(st.session_state.uploaded_files[selected_file]), 
